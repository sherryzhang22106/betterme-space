...
interface FooterProps {
  onNavigate: (view: 'home' | 'service' | 'privacy' | 'disclaimer') => void;
}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid md:grid-cols-4 gap-12 mb-16">
          ...
          <div>
            ...
            <ul className="space-y-4">
              <li><button onClick={() => onNavigate('home')} className="text-slate-500 hover:text-brand-primary text-sm transition-colors">性格分类</button></li>
              <li><button onClick={() => onNavigate('home')} className="text-slate-500 hover:text-brand-primary text-sm transition-colors">内在探索</button></li>
              <li><button onClick={() => onNavigate('home')} className="text-slate-500 hover:text-brand-primary text-sm transition-colors">认知实验</button></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-4">
              <li><button onClick={() => onNavigate('service')} className="text-slate-500 hover:text-brand-primary text-sm transition-colors">服务协议</button></li>
              <li><button onClick={() => onNavigate('privacy')} className="text-slate-500 hover:text-brand-primary text-sm transition-colors">隐私政策</button></li>
              <li><button onClick={() => onNavigate('disclaimer')} className="text-slate-500 hover:text-brand-primary text-sm transition-colors">免责声明</button></li>
            </ul>
          </div>
        </div>
        ...
        <div className="pt-10 border-t border-slate-50 text-center">
          <p className="text-xs text-slate-400">
            &copy; {new Date().getFullYear()} BetterMe Space | 觅我空间. All rights reserved. 沪ICP备XXXXXXXX号
          </p>
        </div>
      </div>
    </footer>
