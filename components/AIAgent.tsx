  ...
  const handleAsk = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `作为一个性格行为学专家与内在认知向导，基于用户的描述：“${input}”，推荐最适合TA的性格或状态测评（如：内耗分析、人生剧本探索、依恋风格等），并给出一段精炼且温暖的内在建议。`,
        config: {
          systemInstruction: '你是一个专业的内在认知向导，名字叫“小觅”。你的回答应该温暖、客观、充满洞察力。严禁使用“心理咨询”、“心理治疗”等医疗词汇。字数控制在150字以内。',
        }
      });
  ...
  return (
    <section className="py-24 overflow-hidden relative transition-all duration-700 bg-slate-50/50 backdrop-blur-sm">
      <div className="absolute top-0 left-0 w-full h-full opacity-30 pointer-events-none overflow-hidden">
         <div className="absolute -top-40 -left-20 w-[600px] h-[600px] rounded-full filter blur-[120px] transition-all duration-1000 animate-pulse" 
              style={{ backgroundColor: 'var(--brand-primary)' }}></div>
         <div className="absolute -bottom-40 -right-20 w-[600px] h-[600px] rounded-full filter blur-[120px] transition-all duration-1000 animation-delay-2000 animate-pulse" 
        ...
        <div className="bg-white/30 backdrop-blur-[60px] border border-white/60 p-10 md:p-14 rounded-[4rem] shadow-[0_24px_48px_-12px_rgba(0,0,0,0.03)] transition-all duration-700 relative overflow-hidden group">
          ...
          <div className="flex justify-center">
            <button
              ...
              style={{ 
                background: isLoading ? '#cbd5e1' : 'linear-gradient(135deg, var(--brand-primary) 0%, var(--brand-secondary) 100%)',
              }}
            >
            ...
            <div className="mt-14 animate-in fade-in slide-in-from-bottom-8 duration-1000">
              <div className="p-10 bg-white/60 border border-white/80 rounded-[3rem] shadow-sm relative overflow-hidden group/result">
                ...
                <div className="flex flex-col md:flex-row items-center md:items-start md:space-x-8">
                  <div className="mb-6 md:mb-0 w-20 h-20 rounded-[1.5rem] flex-shrink-0 flex items-center justify-center font-bold text-white transition-all duration-700 shadow-xl shadow-brand-primary/20 rotate-6 group-hover/result:rotate-0"
                       style={{ backgroundColor: 'var(--brand-primary)' }}>
                     <span className="text-3xl">觅</span>
                  </div>
                  <div className="flex-1 text-center md:text-left">
                    <div className="inline-block px-3 py-1 rounded-lg bg-slate-50 text-[10px] font-black uppercase tracking-[0.25em] mb-4 transition-colors duration-700"
